<%inherit file="/concerto_gui/templates/toplayout.mak" />
...
$(function() {
        % if not(context.get("is_error_template")):
            concerto_gui_ajax({ url: window.location.pathname + window.location.hash,
        ...
        $(window).on("popstate", function(e) {
                ...
                concerto_gui_ajax({url: e.originalEvent.state, concerto_gui: { history:false, target: ConcertoAjaxTarget.TAB }});
        });
        ...
        $(document).on("click", "a:not(.ajax-bypass), area:not(.ajax-bypass)", function(event) {
                ...
                concerto_gui_ajax({ url: url, concerto_gui: { target: $(this).hasClass("no-widget") ? ConcertoAjaxTarget.TAB : ConcertoAjaxTarget.AUTO }});
                return false;
        });
        ...
        $(document).on("submit", "body form", function(event) {
                ...
                var options = {
                        ...
                        data: data,
                        concerto_gui: { target: $(this).hasClass("no-widget") ? ConcertoAjaxTarget.TAB : ConcertoAjaxTarget.AUTO },
                        success: function() {
...
<div id="topmenu">
  ...
  <div id="topmenu_right">
    <button type="button" class="btn btn-info btn-sm concerto_gui-config-button" title="${ _('View options') }" data-toggle="collapse" data-target=".concerto_gui-view-config" disabled>
      <i class="fa fa-cog fa-lg fa-fw"></i>
    </button>
    <button type="button" class="btn btn-info btn-sm concerto_gui-help-button" title="${ _('View help') }" disabled>
...
<%block name="main_content">
    ...
    <div id="main" class="content concerto_gui-resources-container">
