...
from concerto_gui import error, hookmanager
from concerto_gui.compat import STRING_TYPES
...
class MenuManager(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            raise error.ConcertoUserError(N_("Menu error"), N_("The provided YAML menu is invalid"), details=e)

        if not self._menus:
            raise error.ConcertoUserError(N_("Menu error"), N_("Empty menu"))

        default_menu = False

        for menu in self._menus:
            if "name" not in menu and "icon" not in menu:
                raise error.ConcertoUserError(N_("Menu error"), N_("Menu without a name in %s", filename))

            if menu.get("default"):
                if default_menu:
                    raise error.ConcertoUserError(N_("Menu error"), N_("Multiple default menus"))
                ...
                for section in category["sections"]:
                    if "default_tab" in section:
                        if self._default_view:
                            raise error.ConcertoUserError(N_("Menu error"), N_("Multiple default views"))
