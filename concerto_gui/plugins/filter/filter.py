...
from concerto_gui import database, error, hookmanager, resource, response, template, view
from concerto_gui.dataprovider import CriterionOperator
from concerto_gui.utils import AttrObj, json
from concerto_gui.utils.viewhelpers import GridParameters
...
class FilterDatabase(database.DatabaseHelper):
    def get_filters(self, user, ftype=None):
        l = self.query("SELECT id, name, category, description, value FROM Concerto_Filter "
    ...
    def get_filter(self, user, name=None, id_=None):
        query = "SELECT id, name, category, description, value FROM Concerto_Filter WHERE userid = %(user)s"
        if name:
    ...
    def upsert_filter(self, user, filter_):
        ...
        self.upsert("Concerto_Filter", ("userid", "id", "name", "category", "description", "value"), [values], pkey=("id",))

    def delete_filter(self, user, name=None, id_=None):
        query = "SELECT id, name FROM Concerto_Filter WHERE userid = %(user)s"
        if name:
            query += " AND name = %(name)s"
        if id_:
            query += " AND id = %(id)s"
        ...
        rows = self.query(query, user=user.id, name=name, id=id_)
        if rows:
            self.query("DELETE FROM Concerto_Filter WHERE id IN %s", (row[0] for row in rows))

        return rows

    def get_categories(self, user):
        query = "SELECT category FROM Concerto_Filter WHERE userid = %(user)s AND category IS NOT NULL GROUP BY category ORDER BY category"
...
class FilterView(view.View):
    ...
    _filter_menu_tmpl = template.ConcertoTemplate(__name__, "templates/menu.mak")
    ...
    def listing(self):
        ...
        return template.ConcertoTemplate(__name__, "templates/filterlisting.mak").render(**dataset)
    ...
    def _chart_prepare(self, query, options):
        ...
        if not c:
            raise error.ConcertoUserError(N_("Filter error"), N_("Filter '%s' does not exist", filter_name))
    ...
    def edit(self, name=None):
        ...
        if name or id_:
            ...
            if not dataset["fltr"]:
                raise error.ConcertoUserError(N_("Filter error"), N_("Filter '%s' does not exist", name or id_))
        ...
        return template.ConcertoTemplate(__name__, "templates/filteredition.mak").render(**dataset)
    ...
    def save(self, name=None):
        ...
        if not new_name:
            raise error.ConcertoUserError(N_("Could not save filter"), N_("No name for this filter was provided"))
        elif new_name.startswith("/"):
            raise error.ConcertoUserError(N_("Could not save filter"), N_("The filter name cannot start with a slash"))
        ...
        if new_name != name and self._db.get_filter(env.request.user, new_name):
            raise error.ConcertoUserError(N_("Could not save filter"), N_("The filter name is already used by another filter"))
        ...
        return response.ConcertoResponse({"type": "reload", "target": "#main_menu_ng", "options": {"filter": new_name}})
