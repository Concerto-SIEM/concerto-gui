...
from concerto_gui import compat, error, hookmanager, pluginmanager
from concerto_gui.utils import AttrObj, CachingIterator, json
from concerto_gui.utils.timeutil import parser, tzutc
...
def to_datetime(date):
    try:
        ...
        raise error.ConcertoUserError(N_("Conversion error"),
...
class NoBackendError(error.ConcertoUserError):
    def __init__(self, type_):
        error.ConcertoUserError.__init__(
            self, N_("Backend error"), N_("No backend available for '%s' datatype", type_)
        )


class InvalidPathError(error.ConcertoUserError):
...
class ParserError(error.ConcertoUserError):
...
class ItemNotFoundError(error.ConcertoUserError):
    pass
...
class DataProviderManager(pluginmanager.PluginManager):
    def __init__(self, autoupdate=False):
        pluginmanager.PluginManager.__init__(self, "concerto_gui.dataprovider.type", autoupdate=autoupdate)
        pluginmanager.PluginManager("concerto_gui.dataprovider.backend", autoupdate=autoupdate)
    ...
    def load(self, reloading=False):
        ...
        for plugin in env.pluginmanager["concerto_gui.dataprovider.backend"]:
            ...
            if p.type in self._backends:
                raise error.ConcertoUserError(N_("Configuration error"),
