...
from concerto_gui import dataprovider, error, hookmanager
from concerto_gui.dataprovider import CriterionOperator, utils
from concerto_gui.dataprovider.parsers import lucene
from concerto_gui.utils import json, AttrObj
from concerto_gui.utils.timeutil import timezone, get_timestamp_from_datetime
...
class ElasticsearchPlugin(dataprovider.DataProviderBackend):
    def __init__(self):
        ...
        for conf in env.config.elasticsearch:
            ...
            if not name:
                raise error.ConcertoUserError(N_("Invalid configuration"),
        ...
        if not self._instances:
            raise error.ConcertoUserError(N_("Missing configuration"),
...
class ElasticsearchClient(object):
    ...
    def _request(self, url, data="", method="POST", **kwargs):
        try:
            ...
            raise error.ConcertoUserError(N_("Request error"), err)
...
class ElasticsearchQuery(object):
    ...
    def _set_criteria(self, criteria, query=None):
        ...
        if field == "_raw_query":
            ...
            if op is None:
                raise error.ConcertoUserError(N_("Elasticsearch database operator error"),
    ...
    def _add_raw_query(self, query):
        if self._query_string["query_string"]["query"]:
            raise error.ConcertoError(N_("Only one _raw_query path can be specified for the selection"), N_("Elasticsearch database path selection error"))
    ...
    def __init__(self, name, conf, fields):
        ...
        if self.time_format:
            try:
                ...
                raise error.ConcertoUserError(N_("Invalid configuration"), err)
