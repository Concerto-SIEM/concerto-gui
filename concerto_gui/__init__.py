...
from concerto_gui import mainmenu, siteconfig, utils, view
...
try:
    ...
    if not os.access(siteconfig.tmp_dir, os.R_OK | os.W_OK | os.X_OK):
        raise Exception("Concerto temporary directory '%s' lack u+rwx permissions" % siteconfig.tmp_dir)

    if stat.S_IMODE(os.stat(siteconfig.tmp_dir).st_mode) & stat.S_IRWXO:
        raise Exception("Concerto temporary directory '%s' is world accessible" % siteconfig.tmp_dir)
