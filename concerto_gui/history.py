...
from concerto_gui import crontab, database, log, utils
...
class HistoryDatabase(database.DatabaseHelper):
    ...
    def get_queries(self, user, form):
        query = ("SELECT query FROM Concerto_History_Query %s ORDER BY timestamp DESC" %
    ...
    def save(self, user, form, query):
        ...
        self.upsert("Concerto_History_Query", ("userid", "formid", "query", "query_hash", "timestamp"), rows, pkey=("userid", "formid", "query_hash"))
    ...
    def delete(self, user, form, query=False):
        ...
        self.query("DELETE FROM Concerto_History_Query" + self._where(query_hash=query_hash), user=user.id, query_hash=query_hash, form=form)
    ...
    def _history_cron(self, job):
        ...
        query = "SELECT userid, formid, COUNT(query) FROM Concerto_History_Query GROUP BY userid, formid"
        for userid, formid, count in self.query(query):
            if int(count) <= size:
                continue

            rows = self.query("SELECT query FROM Concerto_History_Query WHERE userid = %s AND formid = %s "
                              "ORDER BY timestamp DESC LIMIT %s", userid, formid, size)
            self.query("DELETE FROM Concerto_History_Query WHERE userid = %s AND formid = %s AND query NOT IN %s",
