...
var ConcertoAjaxTarget = Object.freeze({"AUTO": 1, "TAB": 2, "LAYOUT": 3});
...
$(function() {
    $.xhrPool = function() {
        ...
        that.push = function(xhr, settings) {
            if ( settings.concerto_gui.target == ConcertoAjaxTarget.LAYOUT )
                return;
    ...
    $.ajaxSetup({
        concerto_gui: { spinner: true,
                    error: true,
                    target: ConcertoAjaxTarget.AUTO,
                    history: true,
                    bypass: false
        }
    });
    ...
    $(document).ajaxSend(function(event, xhr, settings) {
        ...
        if ( settings.concerto_gui.spinner )
            $("#ajax-spinner").show();
    ...
    $(document).ajaxSuccess(function(event, xhr, settings, data) {
        $.xhrPool.done(xhr);
        if ( ! settings.concerto_gui.bypass )
            concerto_gui_process_ajax_response(settings, data, xhr);
    });
    ...
    $(document).ajaxError(function(event, xhr, settings) {
         ...
         */
        if ( xhr._aborted || ! settings.concerto_gui.error )
            return;

        if ( ! xhr.responseText )
            $("#concerto_gui-dialog-connection-error").modal();

        else if ( xhr.getResponseHeader("content-type") == "application/json" )
            concerto_gui_json_dialog(JSON.parse(xhr.responseText));

        else
            concerto_gui_json_dialog({content: xhr.responseText, error: true});
    });

    $(window).on('resize', concerto_gui_resizeTopMenu);
});


function concerto_gui_location() {
...
function concerto_gui_drawTab(data)
{
    var form;
    ...
    if ( ! form.length )
        form = content = content.wrapAll('<form method="POST" action="' + concerto_gui_location().pathname + '"></form>').parent();
    ...
    concerto_gui_resource_destroy($("#main"));
    $("#main").off().html(content);

    $("#topmenu_right .concerto_gui-help-button").data("href", data._extensions.help).prop("disabled", data._extensions.help ? false : true);
    $("#topmenu_right .concerto_gui-config-button").prop("disabled", $("#main .concerto_gui-view-config").length == 0);

    concerto_gui_resizeTopMenu();
...
function _url_update(xhr, settings)
{
        ...
        if ( redirect || (settings.concerto_gui.history && (settings.type || "").toUpperCase() != "POST") ) {
...
function _process_widget(data, widget)
{
        ...
        if ( data._extensions.help ) {
            var help = $("<button>", { "type": "button", "class": "close concerto_gui-help-button", "data-href": data._extensions.help, "html": '?&nbsp;' });
            $(widget).find(".modal-header button").after(help);
        }

        $(widget).addClass("concerto_gui-resources-container");
        return concerto_gui_json_dialog({"content": widget });
}
...
function _process_ajax_response(settings, data, xhr)
{
    ...
    var event = jQuery.Event("concerto_gui-ajax-response");
    ...
    if ( data._extensions ) {
        $.each(data._extensions.notifications, function(_, value) {
            concerto_gui_notification(value);
        });
    }

    if ( data.type == "reload" ) {
        if ( data.target == "window" )
            $("#concerto_gui-notifications-container > div").promise().done(function() { location.reload(); });
        ...
        if ( data.target ) {
            ...
            if ( settings.concerto_gui.target == ConcertoAjaxTarget.AUTO && widget.length > 0 ) {
                result = _process_widget(data, widget);
            } else {
                _url_update(xhr, settings);
                result = concerto_gui_drawTab(data);
            }
        }
    }
...
function concerto_gui_process_ajax_response(settings, data, xhr)
{
    if ( ! data )
        return;
...
function concerto_gui_ajax(settings)
{
...
function concerto_gui_EventSource(config)
{
    ...
    if ( config['error'] == undefined ) {
        config['error'] = function(e) {
            if ( e.data )
                concerto_gui_json_dialog(JSON.parse(e.data));
            else
                $("#concerto_gui-dialog-connection-error").modal();
...
{
    if ( ! location )
        location = concerto_gui_location().href;


    var ajax_opts = {
        method: method,
        url: location + "/ajax_parameters_update",
        concerto_gui: { spinner: false },
        data: data
    };

    return concerto_gui_ajax(_mergedict(ajax_opts, options));
}


function concerto_gui_update_parameters(data, location, options)
{
    return _update_parameters(data, location, "PATCH", options);
}


function concerto_gui_save_parameters(data, location, options)
{
