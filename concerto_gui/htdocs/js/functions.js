...
$(function() {
  ...
  $.fn.select2_container = function(options) {
      options = options || {};
      options.dropdownParent = this.closest(".concerto_gui-resources-container");
      return this.select2(options);
  };

  $(document).on("reload", "#main", function() {
      return concerto_gui_ajax({
          url: concerto_gui_location().href,
          concerto_gui: {target: ConcertoAjaxTarget.TAB}
      });
  });
  ...
  $(document).on("click", ".concerto_gui-help-button", function(event) {
    /*
     * Prevent default link event handler execution
     */
    event.stopImmediatePropagation();

    window.open($(this).data("href"), "_concerto_gui_help", "width=600,height=600,location=no,menubar=no,toolbar=no,scrollbars=yes").focus();
    return false;
  });
...
// Repeatable entries
  ...
  $(document).on('click', '[data-confirm]', function() {
      ...
      function confirm_handler() {
          ...
          $('#concerto_gui-dialog-confirm').modal('hide');
          return false;
      }

      // Remove the handler in case it was already set. And set the correct handler
      $('#concerto_gui-dialog-confirm-OK').off("click").on("click", confirm_handler);

      concerto_gui_dialog({message: confirm, type: "confirm"});
      return false;
  });
  ...
  $(document).on('click', '.concerto_gui-notification .close', function() {
  ...
  $(document).on('hidden.bs.modal', '.ajax-modal', function () {
    concerto_gui_resource_destroy($(this));
...
function concerto_gui_resizeTopMenu() {
...
function concerto_gui_notification(data)
{
    var notification = $("#concerto_gui-notification").clone().removeAttr("id");
    ...
    $("#concerto_gui-notifications-container").append($(notification));
...
function _is_error_duplicate(content)
{
    ...
    $("#concerto_gui-dialog-container").find(".error-dialog").each(function(i, elem) {
...
function concerto_gui_json_dialog(data, opts)
{
    var dialog;
    ...
    $("#concerto_gui-dialog-container").append(content);
    dialog = $("#concerto_gui-dialog-container > :last-child");
...
function concerto_gui_dialog(data)
{
    if ( typeof(data.type) == 'undefined' )
        data.type = "standard";

    var dialog = $("#concerto_gui-dialog-" + data.type);
...
function concerto_gui_dialog_getMaxHeight() {
    return $(window).height() - $("#topmenu").height() - 100;
}


function concerto_gui_getRenderSize(container, options)
{
        var parent;
        var size = [0, 0];
...
var CONCERTO_GRID_AJAX_PARAMETERS = ["sort_index", "sort_order", "page", "rows"];


function concerto_gui_grid(table, settings) {
    var conf = _mergedict({
        datatype: "local",
...
function concerto_gui_autocomplete(selector, options) {
    options = options || {};
    $(selector).select2_container({
        ...
        ajax: {
            ...
            concerto_gui: {spinner: false, error: false}
        }
    });
}
...
function Criterion(left, operator, right) {
    ...
    ret.toJSON = function() {
        ...
        return {
            "__concerto_gui_class__": ["Criterion", {"left": left, "operator": operator, "right": right}]
        };
    };
...
function _resource_register(obj, target)
{
    ...
    mlist = target.data("concerto_gui-resources");
    if ( ! mlist ) {
        mlist = [];
        target.data("concerto_gui-resources", mlist);
    }
...
function concerto_gui_resource_register(obj)
{
    var target;
    var base = obj.container;

    if ( ! base )
        base = $('.concerto_gui-resources-container:not(.closing) script').last();

    target = $(base).closest(".concerto_gui-resources-container:not(.closing)");
    if ( target.length > 0 )
...
function concerto_gui_resource_destroy(target)
{
    var mlist = target.data("concerto_gui-resources");

    /*
     * removeData() only remove data from the JQuery cache. The data
     * are still present and can still be fetched.
     */
    target.data("concerto_gui-resources", "");
    if ( ! mlist )
        return;
...
function concerto_gui_import(settings)
{
