...
from concerto_gui import compat, csrf, error, hookmanager, log, mainmenu, pluginmanager, registrar, response, template, usergroup, utils
...
class InvalidParameterError(error.ConcertoUserError):
    def __init__(self, name):
        error.ConcertoUserError.__init__(self, N_("Parameters Normalization failed"),
                                         N_("Parameter '%s' is not valid", name),
                                         log_priority=log.WARNING)


class InvalidParameterValueError(error.ConcertoUserError):
...
class MissingParameterError(error.ConcertoUserError):
    def __init__(self, name):
        error.ConcertoUserError.__init__(self, N_("Parameters Normalization failed"),
...
class InvalidMethodError(error.ConcertoUserError):
    def __init__(self, message, log_priority=None, **kwargs):
        error.ConcertoUserError.__init__(self, N_("Invalid method"), message, log_priority=log.ERROR, **kwargs)


class InvalidViewError(error.ConcertoUserError):
    code = 404

    def __init__(self, message, log_priority=None, **kwargs):
        error.ConcertoUserError.__init__(self, N_("Invalid view"), message, log_priority=log.ERROR, **kwargs)
...
class ViewResponse(response.ConcertoResponse):
    def __init__(self, data, **kwargs):
        response.ConcertoResponse.__init__(self, {"type": "content", "content": data})
...
class _ViewDescriptor(object):
    ...
    def respond(self):
        ...
        elif not(resp) or not issubclass(resp.__class__, response.ConcertoResponse):  # Any other type (eg: dict)
            resp = response.ConcertoResponse(resp)
...
class ViewManager(registrar.DelayedRegistrar):
    ...
    def get_view_by_path(self, path, method=None, check_permissions=True):
        ...
        if check_permissions and not rule._concerto_gui_view.check_permissions(env.request.user, view_kwargs=view_kwargs):
            raise usergroup.PermissionDeniedError(rule._concerto_gui_view.view_permissions, path)

        return rule._concerto_gui_view, view_kwargs

    def get_baseview(self):
        # Import here, because of cyclic dependency
        from concerto_gui import baseview
    ...
    def load_views(self, autoupdate=False):
        ...
        pluginmanager.PluginManager("concerto_gui.views", autoupdate).load()
