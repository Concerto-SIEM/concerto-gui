...
from concerto_gui import auth, error, hookmanager, localization, log, resource, response, template, usergroup, view
from concerto_gui.utils.viewhelpers import GridAjaxResponse, GridParameters
from concerto_gui.database import use_transaction
...
class GenericListing(view.View):
    _template = template.ConcertoTemplate(__name__, "templates/userlisting.mak")
...
class GenericListingAjax(view.View):
    ...
    def search(self):
        ...
        return response.ConcertoResponse([{"value": obj.name, "label": obj.name} for obj in objects[:limit]])
...
class UserListingAjax(GenericListingAjax):
    ...
    def delete(self):
        if not env.request.user.has("USER_MANAGEMENT"):
            raise error.ConcertoUserError(N_("Permission Denied"), N_("Access denied to users modification"), log_priority=log.WARNING)
...
class GroupListingAjax(GenericListingAjax):
    ...
    def delete(self):
        if not env.request.user.has("GROUP_MANAGEMENT"):
            raise error.ConcertoUserError(N_("Permission Denied"), N_("Access denied to groups modification"), log_priority=log.WARNING)
...
class SettingsCommon(view.View):
    ...
    _template = template.ConcertoTemplate(__name__, "templates/usersettings.mak")
...
class UserSettings(SettingsCommon):
    ...
    def save(self, name):
        ...
        if not env.auth.has_user(user):
            raise error.ConcertoUserError(N_("Invalid user"), N_("Specified user does not exist"), log_priority=log.WARNING)

        if name != env.request.user.name and not env.request.user.has("USER_MANAGEMENT"):
            raise error.ConcertoUserError(N_("Permission Denied"), N_("Cannot modify other users settings"), log_priority=log.WARNING)

        if not env.request.parameters["language"] in localization.get_languages():
            raise error.ConcertoUserError(N_("Invalid Language"), N_("Specified language does not exist"), log_priority=log.WARNING)

        if not env.request.parameters["timezone"] in localization.get_timezones():
            raise error.ConcertoUserError(N_("Invalid Timezone"), N_("Specified timezone does not exist"), log_priority=log.WARNING)
        ...
        if "password_new" in env.request.parameters and env.request.parameters["password_new"] != env.request.parameters.get("password_new_confirmation"):
            raise error.ConcertoUserError(N_("Password error"), N_("Password mismatch"))
        ...
        if "password_new" in env.request.parameters:
            if modify_self:
                try:
                    ...
                    raise error.ConcertoUserError(N_("Password error"), N_("Invalid password specified"))
        ...
        if reload_type > ReloadEnum["none"]:
            return response.ConcertoResponse({"type": "reload", "target": reload_type.name})
    ...
    def create(self):
        ...
        if not login:
            raise error.ConcertoUserError(N_("Could not create user"), N_("Username required"))

        if login.startswith("/"):
            raise error.ConcertoUserError(N_("Could not create user"), N_("Username cannot start with a slash"))

        user = usergroup.User(login)
        if env.auth.has_user(user):
            raise error.ConcertoUserError(N_("Could not create user"), N_("User %s already exists", login))

        user.create()
        self.save(login)
        return response.ConcertoResponse({"type": "reload", "target": ".commonlisting"})
    ...
    def edit(self, name=None, widget=True):
        ...
        if not name:
            ...
            if not env.auth.has_user(user):
                raise error.ConcertoUserError(_("Invalid User"), N_("Requested user '%s' does not exist", user))

            if user != env.request.user and not env.request.user.has("USER_MANAGEMENT"):
                raise error.ConcertoUserError(_("Permission Denied"), N_("Access denied to other users settings"), log_priority=log.WARNING)
...
class GroupSettings(SettingsCommon):
    ...
    def edit(self, name=None, widget=True):
        ...
        if name:
            if not env.request.user.has("GROUP_MANAGEMENT"):
                raise error.ConcertoUserError(_("Permission Denied"), N_("Access denied to group settings"), log_priority=log.WARNING)
    ...
    def create(self):
        ...
        if not group_name:
            raise error.ConcertoUserError(N_("Could not create group"), N_("No group name provided"))

        if group_name.startswith("/"):
            raise error.ConcertoUserError(N_("Could not create group"), N_("Group name cannot start with a slash"))

        group = usergroup.Group(group_name)
        if env.auth.has_group(group):
            raise error.ConcertoUserError(N_("Could not create group"), N_("Group %s already exists", group_name))
