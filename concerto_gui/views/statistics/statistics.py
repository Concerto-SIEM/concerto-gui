...
from concerto_gui import error, hookmanager, localization, mainmenu, resource, statistics, template, view
from concerto_gui.statistics import Query
...
class GenericStats(view.View):
    ...
    widget_template = template.ConcertoTemplate(__name__, "templates/widget.mak")
    ...
    def get_graph_data(self, category, rtype, title, query, **kwargs):
        ...
        if not class_:
            raise error.ConcertoUserError(N_("Statistics error"), N_("Could not find chart class: %s", kwargs.get("class") or category))
...
class StaticStats(GenericStats):
    view_template = template.ConcertoTemplate(__name__, "templates/statistics.mak")
    ...
    def _load_widget(self):
        ...
        if categories:
            if widget["category"] not in categories:
                raise error.ConcertoUserError(N_("Invalid widget"), N_("The widget category does not exist"))
