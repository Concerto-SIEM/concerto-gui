...
var Dashboard = function(options) {
    ...
    var current_url = options.current_url || concerto_gui_location().href;
    ...
    $(selector).on('click', '.delete', function() {
        ...
        var id = $(this).data("id");
        concerto_gui_resource_destroy($('#' + id));
    ...
    $(selector).on('click', '.message_list_nav_button > a', function(e) {
        ...
        _ajax({
            ...
            success: function(json) {
                ...
                wbody.find('.concerto_gui-view-config').remove();
            },
    ...
    $('.export-grid').on('click', function() {
        ...
        concerto_gui_ajax({ url: current_url, data: {'export': true} });
    });
        ...
        concerto_gui_import({
            ...
            callback: function(total, name, data) {
                try {
                    ...
                    concerto_gui_dialog({message: labels["Failed to import the dashboard."], classname: "danger"});
                    return;
                }
    ...
    $(selector).on('addWidget', function(event, data) {
        if ( data.id ) {
            concerto_gui_resource_destroy($('#' + data.id));
            data.reload = true;
        }
        else {
            _init_widget(data);
    ...
    function _ajax(settings)
    {
        $.ajax($.extend({ type: 'POST',
                          url: current_url,
                          concerto_gui: {error: false, spinner: false, bypass:true}}, settings || {}));
    }
                ...
                if ( winfos.category == 'view' ) {
                    ...
                    _ajax({
                        ...
                        success: function(data) {
                            ...
                            $('.pbody-' + winfos.id + ' .concerto_gui-view-config').remove();
