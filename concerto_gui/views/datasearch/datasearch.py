...
from concerto_gui.dataprovider import Criterion, ResultObject
from concerto_gui.dataprovider.pathparser import PathParser
from concerto_gui.dataprovider.parsers import criteria, lucene
from concerto_gui.localization import format_datetime, format_timedelta
from concerto_gui.renderer import RendererItem
from concerto_gui.statistics import ChronologyChart, DiagramChart, Query
...
class QueryParser(object):
    ...
    def _prepare_groupby_query(self, groupby, orderby):
        ...
        if len(tgroup) > 1:
            raise error.ConcertoUserError(N_("Time group error"), N_("Only one time unit can be specified in a groupby query"))
    ...
    def diagram(self, cview, step=None, **kwargs):
        ...
        except error.ConcertoUserError:
...
class DataSearch(view.View):
    ...
    def _get_dataset(self):
        return template.ConcertoTemplate(__name__, "templates/forensic.mak").dataset()
    ...
    def ajax_timeline(self):
        ...
        return response.ConcertoResponse(resource.HTMLSource("""
            %s
    ...
    def ajax_details(self):
        tmpl = template.ConcertoTemplate(__name__, "templates/details.mak")
        return response.ConcertoResponse(tmpl.dataset(fields_info=self.fields_info,
                                                      fields_value=env.request.parameters))

    def _get_common_infos(self):
        tmpl = template.ConcertoTemplate(__name__, "templates/infos.mak")
    ...
    def ajax_infos(self):
        ...
        return response.ConcertoResponse({"infos": infos})
