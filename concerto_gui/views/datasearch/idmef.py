...
from concerto_gui import dataprovider, resource, response, template, utils, view
from concerto_gui.localization import format_datetime
...
class IDMEFDataSearch(datasearch.DataSearch):
    ...
    def delete(self):
        ...
        if criteria:
            ...
            return response.ConcertoResponse({"type": "reload", "target": "#main", "options": {"current": False}})
    ...
    def ajax_details(self):
        ...
        return response.ConcertoResponse(template.ConcertoTemplate(__name__, "templates/details.mak").dataset(fields_info=sorted(out.keys(), key=utils.path_sort_key), fields_value=out))
