...
from concerto_gui import cli, database, error, response, template, utils, version, view
...
class AboutPlugin(view.View):
    ...
    _all_plugins = ((N_("Apps: API"), "concerto_gui.plugins"),
                    (N_("Apps: View"), "concerto_gui.views"),
                    (N_("Apps: Dataprovider backend"), "concerto_gui.dataprovider.backend"),
                    (N_("Apps: Dataprovider type"), "concerto_gui.dataprovider.type"),
                    (N_("Apps: Authentication"), "concerto_gui.auth"),
                    (N_("Apps: Identification"), "concerto_gui.session"),
                    (N_("Apps: Renderer backend"), "concerto_gui.renderer.backend"),
                    (N_("Apps: Renderer type"), "concerto_gui.renderer.type"))
    ...
    def _add_plugin_info(self, data, catname, mod):
        ...
        except error.ConcertoUserError as e:
    ...
    def render_get(self):
        dset = template.ConcertoTemplate(__name__, "templates/aboutplugin.mak").dataset()
    ...
    def enable(self):
        ...
        if upsrt:
            env.db.upsert("Concerto_Module_Registry", ["module", "enabled"], upsrt, pkey=["module"])
            env.db.trigger_plugin_change()

        return response.ConcertoResponse({"type": "reload", "target": "window"})
