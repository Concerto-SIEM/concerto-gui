...
from concerto_gui import error, template, utils, version, view
...
class Custom(view.View):
    ...
    def _get_tabs(self, appli):
        ...
        if len(tab_urls) != len(tabs):
            raise error.ConcertoUserError(N_("Could not load tabs"), N_("A URL should be defined for each tab"))
...
class CustomView(view.View):
    ...
    def render(self):
        ...
        dataset = template.ConcertoTemplate(__name__, "templates/custom.mak").dataset(
