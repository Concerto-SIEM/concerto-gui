...
from concerto_gui import database, log, pluginmanager, usergroup
from concerto_gui.error import NotImplementedError, ConcertoUserError


class AuthError(ConcertoUserError):
    def __init__(self, session, message=N_("Authentication failed"), log_priority=log.ERROR, log_user=None):
        ConcertoUserError.__init__(self, None, message, log_priority=log_priority, log_user=log_user, template=session.template)
...
class _AuthUser(database.DatabaseHelper):
    ...
    def create_user(self, user):
        self.query("INSERT INTO Concerto_User (name, userid) VALUES (%s, %s)", user.name, user.id)
        env.log.info("Created user \"%s\"" % user)

    def delete_user(self, user):
        name = user.name
        self.query("DELETE FROM Concerto_User WHERE userid = %s", user.id)
    ...
    def get_user_by_id(self, id_):
        ret = self.query("SELECT name, userid FROM Concerto_User WHERE userid = %s", id_)
...
class _AuthGroup(object):
    ...
    def get_group_by_id(self, id_):
        ret = self.query("SELECT name, groupid FROM Concerto_Group WHERE groupid = %s", id_)
        return usergroup.Group(*ret[0]) if ret else None

    def create_group(self, group):
        self.query("INSERT INTO Concerto_Group (name, groupid) VALUES (%s, %s)", group.name, group.id)
        env.log.info("Created group \"%s\"" % group)

    def delete_group(self, group):
        name = group.name
        self.query("DELETE FROM Concerto_Group WHERE groupid = %s", group.id)
