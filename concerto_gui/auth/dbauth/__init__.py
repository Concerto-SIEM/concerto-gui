...
from concerto_gui import auth, database, error, usergroup, version
...
class DBAuth(auth.Auth, database.DatabaseHelper):
    ...
    def init(self, config):
        ...
        try:
            ...
            raise error.ConcertoUserError(N_("DBAuth initialization error"), e)
    ...
    def get_user_list(self, search=None):
        query = "SELECT name, userid FROM Concerto_User"
        if search:
    ...
    def get_group_list(self, search=None):
        query = "SELECT name, groupid FROM Concerto_Group"
        if search:
            query += " WHERE name LIKE %s" % self.escape("%%%s%%" % search)
    ...
    def get_user_permissions(self, user, ignore_group=False):
        ...
        if not ignore_group:
            qstr = " UNION \
                     SELECT pgp.permission FROM Concerto_User_Group pug \
                     JOIN Concerto_Group_Permission pgp USING (groupid) \
                     WHERE pug.userid = %s" % (uid)

        return set(r[0] for r in self.query("SELECT pp.permission FROM Concerto_User_Permission pp where pp.userid = %s%s" % (uid, qstr)))

    def get_group_permissions(self, group):
        return set(r[0] for r in self.query("SELECT permission FROM Concerto_Group_Permission WHERE groupid = %s", group.id))
    ...
    def set_user_permissions(self, user, permissions):
        self._set_permissions("Concerto_User_Permission", "userid", user, permissions)

    def set_group_permissions(self, group, permissions):
        self._set_permissions("Concerto_Group_Permission", "groupid", group, permissions)

    # Group specific
    def set_group_members(self, group, users):
        rows = ((group.id, user.id) for user in users)
        self.upsert("Concerto_User_Group", ("groupid", "userid"), rows, merge={"groupid": group.id})

    def get_group_members(self, group):
        return [usergroup.User(*r) for r in self.query("SELECT PU.name, PUG.userid FROM Concerto_User_Group PUG "
                                                       "JOIN Concerto_User PU USING (userid) WHERE groupid = %s", group.id)]

    def set_member_of(self, user, groups):
        rows = ((group.id, user.id) for group in set(groups))
        self.upsert("Concerto_User_Group", ("groupid", "userid"), rows, merge={"userid": user.id})

    def get_member_of(self, user):
        return [usergroup.Group(*r) for r in self.query("SELECT PG.name, PUG.groupid FROM Concerto_User_Group PUG "
                                                        "JOIN Concerto_Group PG USING (groupid) WHERE userid = %s", user.id)]

    def is_member_of(self, group, user):
        return bool(self.query("SELECT groupid from Concerto_User_Group where groupid = %s and userid = %s", group.id, user.id))

    def get_user_permissions_from_groups(self, user):
        return set(r[0] for r in self.query("SELECT pgp.permission FROM Concerto_User_Group pug \
                                             JOIN Concerto_Group_Permission pgp USING (groupid) \
