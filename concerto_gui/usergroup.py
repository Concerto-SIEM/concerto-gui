...
from concerto_gui import compat, error, hookmanager, localization, log, utils
from concerto_gui.utils import cache, json
...
class PermissionDeniedError(error.ConcertoUserError):
    def __init__(self, permissions, view=None):
        ...
        error.ConcertoUserError.__init__(self, N_("Permission Denied"), msg, log_priority=log.WARNING)
...
class User(NameID):
    ...
    def _configuration(self):
        rows = env.db.query("SELECT config FROM Concerto_User_Configuration WHERE userid = %s", self.id)
        if rows:
    ...
    def sync_properties(self):
        if self._orig_configuration is not None and self._orig_configuration != self.configuration:
            ...
            env.db.upsert("Concerto_User_Configuration", ["userid", "config"], [[self.id, json.dumps(self.configuration)]], pkey=("userid",))
    ...
    def delete(self):
        ...
        env.db.query("DELETE FROM Concerto_User_Configuration WHERE userid = %s", self.id)
