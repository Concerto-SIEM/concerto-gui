...
import preludedb
from concerto_gui import compat, error, log, utils, version
...
class DatabaseError(error.ConcertoUserError):
    name = N_("Database error")

    def __init__(self, message, **kwargs):
        error.ConcertoUserError.__init__(self, message=message, **kwargs)
...
class SQLScript(object):
    ...
    def apply(self):
        ...
        if self.type == "install":
            ...
            self.db.query("UPDATE Concerto_Module_Registry SET version=%s WHERE module=%s%s" %
                          (self.db.escape(self.version),
                           self.db.escape(self._full_module_name),
                           self.db._chknull("branch", self.branch)))

        elif self.type == "branch":
            self.db.query("UPDATE Concerto_Module_Registry SET branch=%s, version=%s, enabled=1 WHERE module=%s",
...
class DatabaseUpdateHelper(DatabaseHelper):
    ...
    def _get_update_directories(self):
        for i in pkg_resources.iter_entry_points("concerto_gui.updatedb", self._module_name):
            try:
    ...
    def _get_install_schema(self):
        ...
        if not ret:
            raise error.ConcertoUserError(N_("Database installation error"),
...
class NoDatabase(DatabaseCommon):
    def query(self, *args, **kwargs):
        raise error.ConcertoUserError(N_("Database configuration error"), N_("Only MySQL and PostgreSQL databases are supported at the moment"))
...
class ConcertoDatabase(object):
    def __new__(cls, config):
        settings = {"host": "localhost", "name": "concerto_gui", "user": "concerto_gui", "type": "mysql"}
        settings.update(config.items())

        type = settings["type"]
        if type == "pgsql":
            return ConcertoPgSQLDatabase(settings)

        elif type == "mysql":
            return ConcertoMySQLDatabase(settings)

        else:
            return ConcertoNoDatabase(settings)


class ConcertoDatabaseCommon(DatabaseCommon):
    def __init__(self):
        env.db = self
        dh = DatabaseUpdateHelper("concerto_gui", self.required_version, self.required_branch)
        dh.apply()
    ...
    def modinfos(self):
        try:
            rows = self.query("SELECT module, branch, version, enabled FROM Concerto_Module_Registry")
        except:
            return {}
    ...
    def _get_last_plugin_changed(self):
        rows = self.query("SELECT time FROM Concerto_Module_Changed")[0][0]
    ...
    def trigger_plugin_change(self):
        self.query("UPDATE Concerto_Module_Changed SET time=current_timestamp")


class ConcertoPgSQLDatabase(PgSQLDatabase, ConcertoDatabaseCommon):
    def __init__(self, settings):
        PgSQLDatabase.__init__(self, settings)
        ConcertoDatabaseCommon.__init__(self)


class ConcertoMySQLDatabase(MySQLDatabase, ConcertoDatabaseCommon):
    def __init__(self, settings):
        MySQLDatabase.__init__(self, settings)
        ConcertoDatabaseCommon.__init__(self)


class ConcertoNoDatabase(NoDatabase, ConcertoDatabaseCommon):
    def __init__(self, settings):
