...
from concerto_gui import database, hookmanager, log, pluginmanager, usergroup, utils
from concerto_gui.error import ConcertoUserError, RedirectionError


class _SessionError(ConcertoUserError):
    code = 401

    def __init__(self, login=None, **kwargs):
        ConcertoUserError.__init__(self, log_user=login, **kwargs)
...
class SessionExpired(_SessionError):
    ...
    message = N_("Your session has expired: please sign in again to continue using Concerto")
...
class SessionDatabase(database.DatabaseHelper):
    def create_session(self, sessionid, user, time):
        self.query("INSERT INTO Concerto_Session (sessionid, userid, login, time) VALUES(%s, %s, %s, %s)",
                   sessionid, user.id, user.name, self.datetime(time))

    def update_session(self, sessionid, time):
        self.query("UPDATE Concerto_Session SET time=%s WHERE sessionid=%s", self.datetime(time), sessionid)

    def get_session(self, sessionid):
        rows = self.query("SELECT login, time FROM Concerto_Session WHERE sessionid = %s", sessionid)
        if not rows:
            raise
    ...
    def delete_session(self, sessionid=None, user=None):
        ...
        if sessionid:
            self.query("DELETE FROM Concerto_Session WHERE sessionid = %s", sessionid)
        elif user:
            self.query("DELETE FROM Concerto_Session WHERE userid = %s", user.id)

    def delete_expired_sessions(self, time):
        self.query("DELETE FROM Concerto_Session WHERE time < %s", self.datetime(time))
